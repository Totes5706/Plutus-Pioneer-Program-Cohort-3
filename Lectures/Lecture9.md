...
An example some of you may be familiar with the Cardano-CLI tool. Which people have used to submit transactions, both simple or Plutus scripts on the blockchain. Marlowe takes that a step further and lets you submit Marlowe contracts on the blockchain. 
...
sothere's several use cases and we're just outlining a couple of them here this is not an exclusivelist but gives you an idea of the things that you might be able to do with the Marlowe cli toolfirst it facilitates interaction and development of Marlowe contracts so you can do things likemeasure the transaction size submit transactions to the blockchain test your integration withwallets or debug validators so this is a developer tool it's a number of different modes we'll talkabout but it's for getting into the nitty gritty of operating Marlowe contracts it also gives youearly access to Marlowe capabilities on testnet or even mainnet so you can run Marlowe contracts youdon't need a web browser  you don't need a phone you  can create your own workflow that operates Marlowea third use case is to integrate with your own workflow or toolset so you can wrap the marla clitool and the way that folks have wrapped cardano cli to create services such as libraries faucetsmarketplaces things like that and then of course it can be used for training in the use of Marloweso people can get a deeper understanding of how Marlowe transactions actually work the model runuser interface provides an abstract view of those transactions Marlowe cli provides a very concreterepresentation of them  that's quite close to what's  occurring on chain so we have several ways we canuse Marlowe cli to interact on the blockchain I'm going to mostly focus on the high level methodlisted here but we also have two others the high level method is really for supporting workflowswhere you just want to run  a contract at the command  line it hides some of the details of plutus andcardano nodes and the internals of how input and  state work in Marlowe  contracts so it really focusesjust on the contract you can also run this Marlowe cli to operate the plutus application back in forMarlowe so there you're directly interacting with the plutus application back end which actually hasthree contracts for Marlowe and several endpoints and you cannot you can interact that way this letsyou operate at the even higher level you don't have to do the txo management and things like thatyou're actually letting either cardona wallet or dotalus do that wallet and UTxO  work in the contract operation so this actually very closely mimics the workflow of Marlowe run because Marlowe runis actually using the plutus application backend there's also a low level mode that is really fordevelopers that are mostly debugging or really want like fine grain controlled over each littleoperation involved in running a Marlowe contract so you can control and modify the Marlowe stateconstruct input look at information about the validators datums and redeemers and basically allthe polluted stuff that's  under the hood when you're  running a smart contract like marla this is simpleto interact with it basically installs the way you would install any tool in the cardano ecosystemyou have you know one option which is to use nick so if you're using a mix based system and you havecabal installed there you  just clone the repository  open the shell and install the tool or if you'renot using nyx or nyx os and you have the cabal and ghc tool chains already installed on your machineyou just clone the repository and install Marlowe  cli it will take a little  while if you haven't beendoing cardano development for all the dependencies to download and various packages to build but witha little patience you'll have a working version of marla cli after doing a simple installationthe basic interface for a command line tool is the command line 
