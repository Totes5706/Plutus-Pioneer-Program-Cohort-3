...
We have the main Marlowe script, and then we also have this subsidiary script just for doing payouts to pay to participants. This is basically a design decision, that was made for security around making payments and authenticating ownership of the funds that will be disbursed. The participants usually have a role token, however this is not always the case and there is a way to use Marlowe contracts without role tokens. The standard use case does use role tokens, and they are basically a Cardano native token that uses the currency symbol of the contract where the token name is the name of the participant; and you will see how this works later on.

the participants as I mentioned are withdrawing funds from this payout script and it enforces that the funds go to the correct person so now we'll plunge into an example I thought an escrow example might be a nice example the high-level workflow it involves three participants we have a buyer that wishes to purchase an item let's say they're purchasing a bicycle or something they're gonna deposit funds after they deposit funds and receive the item they might say hey there's a problem with this it didn't arrive or it's not as advertised or it doesn't work or something like that so they have that option if they don't report a problem  then the funds just go to  the seller so that's kinda the standard use case and we have a flow chart here where you have this deposit of funds if when the buyer makes their decision they say everything is alright the seller receives their funds sothis is a simple path to the contract but if they do report a problem the seller might also agree that there's a problem so there's no real dispute there and the buyer would just receive the refund but the seller might also dispute that there is a problem so if there's a dispute then a third party comes in the mediator and they just they basically act as the judge and decide who actually was going to receive the funds so those are the three roles we have the decision about reporting a problem where the buyer makes that decision the seller gets to make a decision about whether or not to dispute if there is a  problem and then the mediator  makes a decision about who's right in addition to these decisions the contract has what are called  timeouts in it so if one  of the participants doesn't actually take the action they need to take there's a default action that occurs in the contract so you know like if the buyer  doesn't report a problem  or say everything is alright the contract just doesn't stop there it  actually waits and then it'll  proceed this is one of the  features of Marlowe that prevents funds from getting locked in a contract forever there's basically always a timeout in a contract so if you built this contract in blockly this is what it would look like we kind of split the screen into three little slices because it's  too long the contract is  too long to fit in a single screen and still be readable but you can go look at Marlowe playground you could try building this yourself we basically have  this idea of the deposit  the decision here whether everything is all rightor reporting a problem the next decision by the seller of either confirming or disputing a problem and then a decision by the  mediator dismissed claim  or confirmed claim there are various payouts here depending on the logic in  that flow chart see these  two different payouts and  then you'll also see there are some timeouts in the contract these various deadlines that are over here on the side so this is what the contract looks like in the blockley world we saw how it looks like as a flowchart andin Marlowe format which is basically a Haskell  format you could design the  contract this way in the playground you would see it as just plain text but it's basically doing all the stuff we talked about before and presented as test so I mentioned that we had these different paths through the contract here for the eight ones there an additional four pass through the contract or different outcomes more which have to do with the timeouts and I'm not going to go into that today but the first one is the buyer says everything is alright so we just walk through this path or the buyer can say that things are not alright there's a problem and the seller confirms the problems we go through this highlighted area if the seller disputes then it goes down to the mediator and in the case where the mediator decides for the seller we go down here and the seller receives the funds that were deposited by the buyer the last case here is where the mediator decides for the buyer and the buyer gets a refund so these are the cases we're going to just look at one of these cases in more details we have the three roles here we've given them  names and we're going to  look at the case where the seller receives the fund  after there's a dispute and  the mediator rules for the  seller so this it's thispath here buyer reports a problem seller disputes and the media dismisses  the claim so that the funds go to the seller
