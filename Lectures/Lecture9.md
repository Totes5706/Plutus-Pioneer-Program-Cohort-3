...
those low-level workflows  actually come in two forms  so the first is what we're calling a monolithic workflow where basically on the left here you give it all the information about the contractile bundles that all together and we get what we call this Marlowe file and then using query tools like jq which is a json query tool you can pull out information about the contract so this json file this Marlowe file is kind of a bundle of comprehensive information about the contract and then if you want to go to submit transactions you pull out what you need from it and you can submit transactions here you can actually submit them using cardano cli you could submit them you know using block flaw crost or  some web service for doing  transaction submission but all the Marlowe magic kind of occurs in this stage at the beginning and then you're open to use all the data however you wish there's a more granular workflow that really does the same thing except you have individual commands for combining input data needed to run Marlowe into files or addresses things like that and then once again you  can run these with whatever  tools you want to so a little bit of background of just how basically how Marlowe uses plutus in the cardano ecosystem so Marlowe is a Lotusscript so it's a validator and there's actually one Marlowe validator for all Marlowe contracts so it's basically an interpreter  it's a generic script that can run any Marlowe  contract when the contract is created this is slightly specialized for things like the particular network if you're running on a public or private test net  or on mainnet and what's  the currency symbol for the roles involved in the script so there's a tiny bit of specialization but it's basically a generic validator and in the plusworld you know datum is attached to a UTxO that datum basically gives state information about thatUTxO it restricts how it can be spent and in the  Marlowe world the datum  basically contains the part of the actual contract that needs to be executed still so as you're executing a Marlowe contract unchain the datum or the contract part of the data will get smaller and smaller because there's less and less to do in the future  the datum also contains this nation of the contract which has things like balances a record of the past inputs any values of parameters in the contract so you can really think of datum as the state needed to move the contract forward the redeemer in the Plutus world is what's given to a transaction in order to spend the UTxO that has the data so the redeemer in the Marlowe Sense just has the input for the next step of the contract so you're making a deposit you're making a choice you're making a notification the redeemer would simply have that information and then when the transaction is validated on chain it's going to look at the datum which is the current state and it's going to look at  the input which is in the  redeemer and it's going to run the Plutus script which is Marlowe and decide whether that input was  valid and the contract can  proceed to the next step so I mentioned roles roles  are very important in the  Marlowe validator and you  should really be familiar with native tokens they  have a currency symbol they  have a token name and Marlowe uses those so eachcontract in Marlowe has a unique currency symbol and associated with that currency symbol is what's called a validator script  for making payments so we  have the main Marlowe script and then we also have this subsidiary script just for doing payouts to pay to participants and this is basically a design decision that was made for security around making payments and authenticating you know ownership of the funds that will be disbursed   so the participants usually have a role token this is not always the case there's a way to use  Marlowe contracts without  role tokens but the standard use case uses roletokens and they're basically just a native token a cardano native token that uses the currency symbol of the contract and the token name is the name of  the participant and you'll  see how this works later on
